define([
    ...
    Main.prototype = {
        ...
        update: function() {
            Game.physics.arcade.collide(this.player, this.map.layers.collision, this.player.collide, null, this);
            Game.physics.arcade.overlap(this.player, this.map.layers.object);
        ...
        loadMap: function(map) {
            ...
            if(this.map.entities) {
                this.map.entities.callAll('kill');
            }
            ...
            this.map.tilemap = Game.add.tilemap(map.toString());
            ...
            this.map.tilemap.createFromObjects('Event Object Layer', 9, '', 0, true, false, this.teleporters);

            this.map.tilemap.setCollision(1, true, this.map.layers.collision);
            this.map.tilemap.setTileIndexCallback(182, this.enterBattleCheck, this, this.map.layers.object);

            for(var layer in this.map.layers) {
                if(this.map.layers[layer] instanceof Phaser.TilemapLayer) {
                    this.map.layers[layer].resizeWorld();
                }
            }

            this.map.layers.collision.debug = true;
            this.map.layers.object.debug = true;
        },

        enterBattleCheck: function() {
