define([
    ...
    Phaser.Plugin.ScreenFader.prototype.fadeIn = function() {
        var fader = GameOverlay.add.graphics(0, 0);
        fader.beginFill('rgb(0, 0, 0)', 1);
        fader.drawRect(0, 0, GameOverlay.width, GameOverlay.height);
        fader.alpha = 1;
        fader.endFill();

        var tween = GameOverlay.add.tween(fader);
        tween.to({ alpha: 0 });
        tween.start();
    };

    Phaser.Plugin.ScreenFader.prototype.fadeOut = function(targetNextState, sourceNextState) {
        var fader = GameOverlay.add.graphics(0, 0);
        fader.beginFill('rgb(0, 0, 0)', 1);
        fader.drawRect(0, 0, GameOverlay.width, GameOverlay.height);
        fader.alpha = 0;
        ...
        if(targetNextState) {
            tween.onComplete.add(function() {
                GameOverlay.state.start(sourceNextState);
                Game.state.start(targetNextState);
            }, this);
        }
        tween.start();
    };
